...
echo -e '\033[33m----------Create Drupal Admin Account----------\033[33m'
echo -e "\033[0m"
cd $docroot
read -p "Enter your Admin Username: " adUname
read -p "Enter your Admin Password: " adPass
echo -e '\033[33m----------Installing Drupal site----------\033[33m'
echo -e "\033[0m"
drush si -y --account-name=$adUname --account-pass=$adPass --db-url=mysql://root:dbpassword@localhost/$dbname


#Installing Drupal Module
echo -e '\033[33m----------Installing Drupal Basic Modules----------\033[33m'
echo -e "\033[0m"
...
sudo drush en admin admin_menu ckeditor ctools page_manager ctools_custom_content bulk_export views_content stylizer ctools_plugin_example ctools_ajax_sample ctools_access_ruleset devel devel_generate devel_node_access eva features feeds feeds_news feeds_ui feeds_import library module_filter pathauto token views views_ui views_slideshow views_slideshow_cycle -y
